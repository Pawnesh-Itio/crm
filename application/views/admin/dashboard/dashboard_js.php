...
$(function() {
    ...
    if (projects_chart.length > 0) {
        // Projects statuses
        console.log(<?php echo json_encode($projects_status_stats); ?>);
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        ticks: {
                            stepSize: 1, // Set step size to 1 to ensure integers
...
<!-- Invoice Canvas Chart Started -->
<script>
const invoiceCanvasChart = document.getElementById('invoiceCanvasChart');
const invoiceCount = parseInt(invoiceCanvasChart.dataset.count);
const invoicetotal = parseInt(invoiceCanvasChart.dataset.total);
const invoiceLabels = JSON.parse(invoiceCanvasChart.dataset.labels);
var ctx = document.getElementById('invoiceCanvasChart').getContext('2d');
...
var invoiceChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Invoices',
            data: [invoiceCount,invoicetotal -invoiceCount ], // Total as first value, completed as second
            backgroundColor: [
                'rgba(54, 162, 235, 0.2)', // Completed value (showing in blue)
                'rgba(255, 99, 132, 0.2)'  // Total value (showing in light gray or neutral)
            ],
            borderColor: [
                'rgba(54, 162, 235, 1)',
                'rgba(255, 99, 132, 1)'
    ...
    options: {
        rotation: Math.PI, // Rotate the chart so it starts from the bottom
        cutoutPercentage: 80, // Creates the "meter" effect
        circumference: Math.PI, // Half-circle for the meter style
        tooltips: {
...
<!-- Invoice Canvas Chart Ended -->
<!-- Leads Canvas Chart Started -->
<script>
const leadCanvasChart = document.getElementById('leadCanvasChart');
const leadCount = parseInt(leadCanvasChart.dataset.count);
const leadTotal = parseInt(leadCanvasChart.dataset.total);
const leadLabels = JSON.parse(leadCanvasChart.dataset.labels);
var ctx = document.getElementById('leadCanvasChart').getContext('2d');
var leadChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Leads',
            data: [leadCount, leadTotal - leadCount], // Completed leads vs remaining leads (total - completed)
            backgroundColor: [
                'rgba(54, 162, 235, 0.2)', // Completed leads (showing in blue)
                'rgba(255, 99, 132, 0.2)'  // Remaining leads (showing in light gray)
            ],
            borderColor: [
                'rgba(54, 162, 235, 1)', // Blue border for completed leads
                'rgba(255, 99, 132, 1)'  // Red border for remaining leads
            ],
            borderWidth: 1
        }]
    },
...
<!-- Leads Canvas Chart Ended -->
<!-- Project Canvas Chart Started -->
<script>
const projectCanvasChart = document.getElementById('projectCanvasChart');
const projectCount = parseInt(projectCanvasChart.dataset.count);
const projectTotal = parseInt(projectCanvasChart.dataset.total);
const projectLabels = JSON.parse(projectCanvasChart.dataset.labels);
var ctx = document.getElementById('projectCanvasChart').getContext('2d');
var projectChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Projects',
            data: [projectCount, projectTotal - projectCount], // Show completed vs remaining (total - completed)
            backgroundColor: [
                'rgba(54, 162, 235, 0.2)', // Completed projects (showing in blue)
                'rgba(255, 99, 132, 0.2)'  // Remaining projects (showing in light gray)
            ],
            borderColor: [
                'rgba(54, 162, 235, 1)', // Blue border for completed
                'rgba(255, 99, 132, 1)'  // Red border for remaining
            ],
            borderWidth: 1
        }]
    },
...
<!-- Project Canvas Chart Ended -->
<!-- Task Canvas Chart Started -->
<script>
const taskCanvasChart = document.getElementById('taskCanvasChart');
const taskCount = parseInt(taskCanvasChart.dataset.count);
const taskTotal = parseInt(taskCanvasChart.dataset.total);
const taskLabels = JSON.parse(taskCanvasChart.dataset.labels);
var ctx = document.getElementById('taskCanvasChart').getContext('2d');
var taskChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Tasks',
            data: [taskCount, taskTotal - taskCount], // Completed tasks vs remaining tasks (total - completed)
            backgroundColor: [
                'rgba(54, 162, 235, 0.2)', // Completed tasks (blue)
                'rgba(255, 99, 132, 0.2)'  // Remaining tasks (light red)
            ],
            borderColor: [
                'rgba(54, 162, 235, 1)', // Blue border for completed tasks
                'rgba(255, 99, 132, 1)'  // Red border for remaining tasks
            ],
            borderWidth: 1
        }]
    },
    options: {
        rotation: Math.PI, // Rotate the chart to start from the bottom
        cutoutPercentage: 80, // Creates the "meter" effect
        circumference: Math.PI, // Semi-circular display for meter style
        tooltips: {
            enabled: false // Disable tooltips if not needed
        },
        responsive: true,
