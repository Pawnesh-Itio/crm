...
<div class="modal fade" id="sales_item_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open('admin/invoice_items/manage', 
                   ['id' => 'invoice_item_form',
                   'enctype' => 'multipart/form-data' // Add this line for file upload support
                   ]); ?> 
...
function manage_invoice_items(form) {
    ...
    $.ajax({
        ...
        // console.log(response); // checking response coming from the post request.
        ...
        if (response.success == true) {
            ...
            if ($("body").find('.accounting-template').length > 0) {
                if (!item_select.hasClass('ajax-search')) {
                    ...
                    if (group.length == 0) {
                        var _option = '<optgroup label="' + (response.item.group_name == null ? '' : response.item.group_name) + '" data-group-id="' + response.item.group_id + '"></optgroup>';
